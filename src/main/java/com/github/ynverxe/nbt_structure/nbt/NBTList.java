...
public class NBTList extends NBT<List<NBT>> implements List<NBT>, Cloneable {
  ...
  public NBTList(@NotNull List<NBT> value, @Nullable TagType<?> type) {
    ...
    if (type == TagType.END) throw new IllegalArgumentException("cannot create a list of end tags");
  ...
  public @NotNull List<Object> normalize() {
    return value.stream().map(NBT::normalize).collect(Collectors.toList());
  }
  ...
  private void checkIncomingElement(NBT value) {
    ...
    if (value.type() != type)
      throw new IllegalArgumentException(
          "This list only accepts values of type: " + value.type().id());
  }
